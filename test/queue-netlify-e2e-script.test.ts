...
const utilsPath = '../playground/scripts/queue-netlify-e2e-utils.mjs'

describe('playground queue netlify e2e utils', () => {
  ...
  it('resolves options with precedence CLI > env > config > deploy', async () => {
    ...
    const options = resolveE2EOptions({
      ...
      env: { QUEUE_E2E_BASE_URL: 'https://env.netlify.app', QUEUE_E2E_POLL_TIMEOUT_MS: '4000' },
