...
Agent prompts often need to combine system instructions, user input, file snippets, and history into a single context window. The `context` module helps you estimate tokens, truncate content safely, and assemble a prompt in priority order. Use it when you want predictable prompt size without hand-tuning every string.

## When To Use It

- You build prompts from multiple inputs (messages, files, tool output) and need a token budget.
- You want consistent truncation behavior across different sources.
- You need to split large inputs into chunks that fit your model limits.
