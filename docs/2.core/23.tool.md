...
Tool calling requires a JSON Schema that describes inputs precisely. The `tool` module lets you define tools with typed parameters and convert them into provider-compatible schemas. Use it to keep tool definitions consistent across LLM providers and to avoid hand-written schema objects.

## When To Use It

- You expose tools to an LLM provider that expects JSON Schema.
- You want a single tool definition that can target multiple providers.
- You want type-safe parameters for tools like file I/O, search, or system operations.
