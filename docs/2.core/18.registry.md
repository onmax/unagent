...
Agent systems often support multiple providers for the same capability. The `registry` module helps you register providers, match inputs, and resolve to the best candidate by priority. Use it to keep provider selection explicit and testable.

## When To Use It

- You support multiple model, tool, or storage providers behind one interface.
- You want deterministic resolution with priority and matching rules.
- You need a registry that works in core code and is easy to mock in tests.
