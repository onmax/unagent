...
Skill installation needs a durable record of what is installed, where it came from, and whether files changed. The `lock` module reads and writes `skill.lock` files and provides helpers to update entries safely. Use it to make installs repeatable and to detect drift between runs.

## When To Use It

- You install or uninstall skills and want to keep an inventory.
- You need to compute hashes to detect changes to installed content.
- You want to read or write lockfiles without maintaining your own schema.
