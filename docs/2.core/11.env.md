...
Vercel-compatible agent detection with explicit precedence.

This is useful when you want to match Vercel Labs `determineAgent()` behavior, but get back **unagent agent IDs**.

Credits: based on Vercel’s [`detect-agent`](https://github.com/vercel/vercel/tree/main/packages/detect-agent).

```ts
import { determineAgent } from 'unagent/env'

const agent = determineAgent()
// { id: 'cursor' | 'claude-code' | 'codex' | ..., detectedBy: 'ai_agent' | 'env' | 'fs' }
```

You can force/override detection with `AI_AGENT`:
...
`envDetect` entries support:
- `VAR` (presence)
- `VAR=VALUE` (case-insensitive equality)
