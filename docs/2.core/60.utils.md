...
Agent tooling often needs small helpers for formatting output and working with paths. The `utils` module centralizes these helpers so core modules do not reimplement string and path utilities. Use it for CLI output, logs, and user-facing formatting.

## When To Use It

- You build a CLI and want consistent ANSI coloring and formatting helpers.
- You need small string utilities like pluralization and list formatting.
- You want to normalize or shorten paths for readable output.
