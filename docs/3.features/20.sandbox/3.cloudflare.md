...
const preflight = validateSandboxConfig({ name: 'cloudflare' })
if (!preflight.ok)
  console.error(preflight.issues)
```

Structured `SandboxError` instances include a `code` field (e.g. `NOT_SUPPORTED`, `TIMEOUT`) so callers can map errors to the appropriate HTTP status.
...
Failed checkouts throw a structured `SandboxError` with `code: 'CF_GIT_CHECKOUT_FAILED'`
and `details` including the operation, URL, and requested options.
