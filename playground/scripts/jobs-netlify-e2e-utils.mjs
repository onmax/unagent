...
export function resolveE2EOptions({ args, env, config, deploy }) {
  ...
  const baseUrl = args.baseUrl || env.JOBS_E2E_BASE_URL || config.baseUrl || deployBaseUrl
  const bootstrapCommand = args.bootstrapCommand || env.NETLIFY_JOBS_E2E_CLI || config.bootstrapCommand || DEFAULT_BOOTSTRAP_COMMAND
  const pollTimeoutMs = parsePositiveInt(
    args.pollTimeoutMs ?? env.JOBS_E2E_POLL_TIMEOUT_MS ?? config.pollTimeoutMs,
    DEFAULT_POLL_TIMEOUT_MS,
  )
  const pollIntervalMs = parsePositiveInt(
    args.pollIntervalMs ?? env.JOBS_E2E_POLL_INTERVAL_MS ?? config.pollIntervalMs,
