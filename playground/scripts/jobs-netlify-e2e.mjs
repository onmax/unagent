...
import { normalizeBase, parseArgs, parseBootstrapJson, resolveE2EOptions } from './jobs-netlify-e2e-utils.mjs'
...
function runBootstrapCli(command) {
  ...
  if (res.status !== 0) {
    ...
    throw new Error(`NETLIFY_JOBS_E2E_CLI failed with exit code ${res.status}${snippet ? ` (${snippet})` : ''}`)
  }
  return {
    output: `${stdout}\n${stderr}`.trim(),
...
async function runAll() {
  log('Netlify Jobs E2E runner\n')

  const args = parseArgs(process.argv.slice(2))
  const configPath = args.config || process.env.JOBS_E2E_CONFIG
  ...
  if (!baseUrl) {
    pushResult('config', 'failed', 'Missing base URL. Set JOBS_E2E_BASE_URL, --base-url, config.baseUrl, or return it from bootstrap deploy JSON.')
    log('Summary:')
  ...
  const providerBase = `${base}/api/jobs/netlify`
  const runId = `netlify-jobs-e2e-${Date.now()}-${randomUUID().slice(0, 8)}`
  const jobName = 'playground:receipt'
  let messageId = ''
  let eventName = process.env.NETLIFY_JOBS_EVENT || 'unagent.playground.jobs'
  ...
  try {
    ...
    if (json?.supports?.enqueue !== true)
      throw new Error('expected enqueue support to be true')
    if (!Array.isArray(json?.jobs) || !json.jobs.find(job => job.name === jobName))
      throw new Error(`expected ${jobName} to be listed in supports`)
    if (typeof json?.event === 'string' && json.event.trim())
      eventName = json.event.trim()
    pushResult('supports', 'passed', 'enqueue support and jobs list verified')
  }
  catch (error) {
  ...
  try {
    const { status, json } = await requestJson(`${providerBase}/enqueue`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        name: jobName,
        payload: {
          hello: 'netlify-jobs-e2e',
          runId,
    ...
    if (typeof json?.messageId !== 'string' || !json.messageId)
      throw new Error('missing messageId in enqueue response')
    ...
    pushResult('enqueue', 'passed', `messageId: ${messageId}${json?.baseUrl ? ` baseUrl=${json.baseUrl}` : ''}`)
  }
  catch (error) {
    pushResult('enqueue', 'failed', error instanceof Error ? error.message : String(error))
  }

  try {
    if (!messageId)
      throw new Error('missing messageId from enqueue step')

    const { status } = await requestJson(`${base}/.netlify/functions/jobs-background?events=${encodeURIComponent(JSON.stringify([eventName]))}`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify([{
        eventName,
        eventId: messageId,
        data: { hello: 'netlify-jobs-e2e', runId },
