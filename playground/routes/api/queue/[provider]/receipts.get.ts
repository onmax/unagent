...
import { listNetlifyQueueReceipts, readNetlifyQueueReceipt } from '../../../../server/_shared/netlify-receipts'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (eventId) {
      const receipt = await readNetlifyQueueReceipt(runId, eventId)
      return {
        ok: true,
        provider,
        runId,
    ...
    const receipts = await listNetlifyQueueReceipts(runId, limit)
    return {
      ok: true,
      provider,
      runId,
