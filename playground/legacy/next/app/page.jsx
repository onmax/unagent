...
const defaultQueuePayload = JSON.stringify({ hello: 'vercel' }, null, 2)
...
export default function PlaygroundPage() {
  ...
  const queueSupports = async () => {
    await call('/api/queue/supports')
  }

  const queueSend = async () => {
    let payloadValue
    try {
      payloadValue = queuePayload.trim() ? JSON.parse(queuePayload) : {}
    }
    catch (error) {
      pushLog({
        id: Math.random().toString(36).slice(2),
    ...
    await call('/api/queue/send', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ payload: payloadValue, options: { delaySeconds: 1 } }),
    })
  }
  ...
  return (
    <main style={{ padding: '1rem' }}>
      ...
      <section className="section">
        <h2>Queue</h2>
        <label htmlFor="queuePayload">Payload (JSON)</label>
        <textarea
          id="queuePayload"
          rows={4}
          value={queuePayload}
          onChange={event => setQueuePayload(event.target.value)}
        />

        <div className="buttons">
          <button onClick={queueSupports}>supports</button>
